...
for t in range(19): 
    best  = chromosome(101, (np.random.permutation(nodes))) #used to store best chromosome found in total run
    whenFound = math.inf
    ...
    #Population Size
    gaPopSize = 30
    #maxAge = 45 #Set maximum iterations for which a chromosome may live
    i = 0
    tempArr = []
    populationArr = []
    prevdist = math.inf
    ...
    for i in range(gaPopSize):
        ...
        populationArr.append(chromosome(i, np.append(arr, arr[0])))
    ...
    while(((generationNumber < numGenertations))):
        ...
        for i in (range(gaPopSize)):
            ...
            if(prevdist != populationArr[i].distance):
                #print(populationArr[0].distance)
                currTime = time.time() - start
                bestDist = populationArr[i].distance
                
                if(bestDist < best.distance):
                    best = copy.deepcopy(populationArr[i])
            ...
            prevdist = populationArr[i].distance            
            
            
            ###################################EXPERIMENT 3###########
            #Calculate Artificial fitness - declining fitness by age
            populationArr[i].fitness = distance + (populationArr[i].ageCurrent**3 /1000)     
            ###################################EXPERIMENT 3###########
        ...
        #sort based on fitness -AKA smallest fitness is best
        ...
        for chromo in pairedMates:
            ...
            #Ensure no errors - shouldn't run
            if(len(tempChromosome1) <101):
                tempChromosome1.append(tempChromosome1[0])    
                print("ERROR: chromosome 1 was wrong length")
                print(len(tempChromosome1))

            if(len(tempChromosome2) <101):
                tempChromosome2.append(tempChromosome2[0])  
                print("ERROR: chromosome 2 was wrong length")
                print(len(tempChromosome2))
    ...
    print(str(round(time.time() - start ,2)) + "\t" + str(round(best.distance,2)) + "\t" + str(round(genFound,2)) + "\t" + str(round(whenFound,2)))
